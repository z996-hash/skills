...
- Avoid long exploratory `conda search` calls; use direct bounded install or `--dry-run --json` when possible.
- If the base Python/framework/CUDA decision is wrong, remove and recreate the same final environment name instead of creating `-try` environments.
...
## Binary Library Channel Mismatch
...
- Repair the specific package family in place before rebuilding the environment.
- Avoid mixing `defaults` and `conda-forge` for ABI-sensitive image/scientific libraries.
- For small leaf packages, a compatible pip wheel can be faster than another conda solve.

## Legacy Setuptools / Distutils

Signs:

- `distutils.version`
- `No module named 'distutils'`
- old PyTorch Lightning or TensorBoard imports fail under newer setuptools

Moves:

- Pin `setuptools==59.5.0` for old PyTorch/Lightning stacks.
- Re-run import validation before changing the base framework stack.
...
- `cannot import name ... from transformers`

Moves:

- Pin `transformers` to an older compatible release.
- Prefer targeted package downgrade over full environment rebuild.
...
- Persist validated `CUDA_HOME`, `PATH`, `LD_LIBRARY_PATH`, and `TORCH_CUDA_ARCH_LIST` fixes in conda activate/deactivate scripts.

## PyTorch Extension Runtime Path
...
- `libtorch.so`
- `torch_cpu.dll`
- compiled extension imports only work when a manual library path is exported

Moves:

- Add the environment's `torch/lib` and `lib` directories to runtime library paths.
- Persist the fix in conda activate/deactivate scripts.
- Do not clone or rename the environment after compiling extensions; absolute paths may be embedded.
